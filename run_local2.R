run_local <- function(responses, nfact, start_item, nitems, thetas.start_in, 
  ...
  fn <- function(n, responses, nfact, start_item, nitems, thetas.start_in, 
                 score, verbose, design, test, Info_thetas_cov,theta_SEs){
    person <- Person$new(nfact=nfact, nitems=nitems, theta_SEs=theta_SEs, #sqrt(diag(test@gp$gcov)), #added to user parameter
  ...
  # Set parameter theta_SE inside function
  theta_SEs <- prior_sd #added
  ...
  if(is.null(cl) || nrow(responses) == 1L){
    ret <- if(progress){
      pbapply::pblapply(1L:nrow(responses), fn, responses=responses, nfact=nfact, start_item=start_item,
                        nitems=nitems, thetas.start_in=thetas.start_in, score=score, verbose=verbose, theta_SEs = theta_SEs, #added
    ...
    ret <- if(progress){
      pbapply::pblapply(cl=cl, X=1L:nrow(responses), FUN=fn, responses=responses,
                        ...
                        nitems=nitems, thetas.start_in=thetas.start_in, score=score, theta_SEs = theta_SEs, #added
